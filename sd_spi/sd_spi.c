...
static void sd_spi_bus_to_ground() {
  furi_hal_gpio_init_ex(furi_hal_sd_spi_handle->miso, GpioModeOutputPushPull,
                        GpioPullNo, GpioSpeedVeryHigh, GpioAltFnUnused);
  furi_hal_gpio_init_ex(furi_hal_sd_spi_handle->mosi, GpioModeOutputPushPull,
                        GpioPullNo, GpioSpeedVeryHigh, GpioAltFnUnused);
  furi_hal_gpio_init_ex(furi_hal_sd_spi_handle->sck, GpioModeOutputPushPull,
                        GpioPullNo, GpioSpeedVeryHigh, GpioAltFnUnused);
...
static void sd_spi_bus_rise_up() {
  ...
  furi_hal_gpio_init_ex(furi_hal_sd_spi_handle->miso,
                        GpioModeAltFunctionPushPull, GpioPullUp,
                        GpioSpeedVeryHigh, GpioAltFn5SPI2);
  furi_hal_gpio_init_ex(furi_hal_sd_spi_handle->mosi,
                        GpioModeAltFunctionPushPull, GpioPullUp,
                        GpioSpeedVeryHigh, GpioAltFn5SPI2);
  furi_hal_gpio_init_ex(furi_hal_sd_spi_handle->sck,
                        GpioModeAltFunctionPushPull, GpioPullUp,
                        GpioSpeedVeryHigh, GpioAltFn5SPI2);
}
...
static inline uint8_t sd_spi_read_byte(void) {
  uint8_t responce;
  furi_check(furi_hal_spi_bus_trx(furi_hal_sd_spi_handle, NULL, &responce, 1,
                                  SD_TIMEOUT_MS));
  return responce;
}

static inline void sd_spi_write_byte(uint8_t data) {
  furi_check(furi_hal_spi_bus_trx(furi_hal_sd_spi_handle, &data, NULL, 1,
                                  SD_TIMEOUT_MS));
}

static inline uint8_t sd_spi_write_and_read_byte(uint8_t data) {
  uint8_t responce;
  furi_check(furi_hal_spi_bus_trx(furi_hal_sd_spi_handle, &data, &responce, 1,
                                  SD_TIMEOUT_MS));
  return responce;
}

static inline void sd_spi_write_bytes(uint8_t *data, uint32_t size) {
  furi_check(furi_hal_spi_bus_trx(furi_hal_sd_spi_handle, data, NULL, size,
                                  SD_TIMEOUT_MS));
}

static inline void sd_spi_read_bytes(uint8_t *data, uint32_t size) {
  furi_check(furi_hal_spi_bus_trx(furi_hal_sd_spi_handle, NULL, data, size,
                                  SD_TIMEOUT_MS));
}
...
static inline void sd_spi_write_bytes_dma(uint8_t *data, uint32_t size) {
  ...
  furi_check(furi_hal_spi_bus_trx_dma(furi_hal_sd_spi_handle, data, NULL, size,
                                      SD_TIMEOUT_MS * timeout_mul));
}

static inline void sd_spi_read_bytes_dma(uint8_t *data, uint32_t size) {
  uint32_t timeout_mul = (size / 512) + 1;
  furi_check(furi_hal_spi_bus_trx_dma(furi_hal_sd_spi_handle, NULL, data, size,
                                      SD_TIMEOUT_MS * timeout_mul));
}
...
static SdSpiCmdAnswer sd_spi_send_cmd(SdSpiCmd cmd, uint32_t arg, uint8_t crc,
                                      SdSpiCmdAnswerType answer_type) {
...
static SdSpiStatus sd_spi_init_spi_mode_v1(void) {
  ...
  do {
    ...
    // ACMD41 (SD_APP_OP_COND) to initialize SDHC or SDXC cards: R1 response
    // (0x00: no errors)
    response =
        sd_spi_send_cmd(SD_CMD41_SD_APP_OP_COND, 0, 0xFF, SdSpiCmdAnswerTypeR1);
    sd_spi_deselect_card_and_purge();
...
static SdSpiStatus sd_spi_init_spi_mode_v2(void) {
  ...
  do {
    ...
    // ACMD41 (APP_OP_COND) to initialize SDHC or SDXC cards: R1 response (0x00:
    // no errors)
    response = sd_spi_send_cmd(SD_CMD41_SD_APP_OP_COND, 0x40000000, 0xFF,
                               SdSpiCmdAnswerTypeR1);
    sd_spi_deselect_card_and_purge();
  ...
  if (FLAG_SET(response.r1, SdSpi_R1_ILLEGAL_COMMAND)) {
    ...
    do {
      ...
      // ACMD41 (SD_APP_OP_COND) to initialize SDHC or SDXC cards: R1 response
      // (0x00: no errors)
      response = sd_spi_send_cmd(SD_CMD41_SD_APP_OP_COND, 0, 0xFF,
                                 SdSpiCmdAnswerTypeR1);
      sd_spi_deselect_card_and_purge();
...
static SdSpiStatus sd_spi_init_spi_mode(void) {
  ...
  response =
      sd_spi_send_cmd(SD_CMD8_SEND_IF_COND, 0x1AA, 0x87, SdSpiCmdAnswerTypeR7);
  sd_spi_deselect_card_and_purge();
  ...
  if (FLAG_SET(response.r1, SdSpi_R1_ILLEGAL_COMMAND)) {
    ...
    response =
        sd_spi_send_cmd(SD_CMD58_READ_OCR, 0, 0xFF, SdSpiCmdAnswerTypeR3);
    sd_spi_deselect_card_and_purge();
...
static SdSpiStatus sd_spi_get_csd(SD_CSD *csd) {
  ...
  if (response.r1 == SdSpi_R1_NO_ERROR) {
    if (sd_spi_wait_for_data(SD_TOKEN_START_DATA_SINGLE_BLOCK_READ,
                             SD_TIMEOUT_MS) == SdSpiStatusOK) {
      // read CSD data
      ...
      *************************************************************************/
      ...
      if (sd_high_capacity == 0) {
        ...
        csd->version.v1.DeviceSize = ((csd_data[6] & 0x03) << 10) |
                                     (csd_data[7] << 2) |
                                     ((csd_data[8] & 0xC0) >> 6);
        ...
        csd->version.v1.DeviceSizeMul =
            ((csd_data[9] & 0x03) << 1) | ((csd_data[10] & 0x80) >> 7);
      } else {
        csd->version.v2.Reserved1 =
            ((csd_data[6] & 0x0F) << 2) | ((csd_data[7] & 0xC0) >> 6);
        csd->version.v2.DeviceSize =
            ((csd_data[7] & 0x3F) << 16) | (csd_data[8] << 8) | csd_data[9];
        csd->version.v2.Reserved2 = ((csd_data[10] & 0x80) >> 8);
      }

      csd->EraseSingleBlockEnable = (csd_data[10] & 0x40) >> 6;
      csd->EraseSectorSize =
          ((csd_data[10] & 0x3F) << 1) | ((csd_data[11] & 0x80) >> 7);
      ...
      csd->MaxWrBlockLen =
          ((csd_data[12] & 0x03) << 2) | ((csd_data[13] & 0xC0) >> 6);
...
static SdSpiStatus sd_spi_cmd_read_blocks(uint32_t *data, uint32_t address,
                                          uint32_t blocks,
                                          uint32_t timeout_ms) {
  ...
  SdSpiCmdAnswer response = sd_spi_send_cmd(
      SD_CMD16_SET_BLOCKLEN, SD_BLOCK_SIZE, 0xFF, SdSpiCmdAnswerTypeR1);
  sd_spi_deselect_card_and_purge();
  ...
  while (blocks--) {
    ...
    response = sd_spi_send_cmd(SD_CMD17_READ_SINGLE_BLOCK, block_address, 0xFF,
                               SdSpiCmdAnswerTypeR1);
    if (response.r1 != SdSpi_R1_NO_ERROR) {
      sd_spi_deselect_card_and_purge();
      return SdSpiStatusError;
...
static SdSpiStatus sd_spi_cmd_write_blocks(uint32_t *data, uint32_t address,
                                           uint32_t blocks,
                                           uint32_t timeout_ms) {
  uint32_t block_address = address;
  uint32_t offset = 0;
  ...
  while (blocks--) {
    ...
    response = sd_spi_send_cmd(SD_CMD24_WRITE_SINGLE_BLOCK, block_address, 0xFF,
                               SdSpiCmdAnswerTypeR1);
    if (response.r1 != SdSpi_R1_NO_ERROR) {
      sd_spi_deselect_card_and_purge();
      return SdSpiStatusError;
...
uint8_t sd_max_mount_retry_count() { return 10; }
...
SdSpiStatus sd_init(bool power_reset) {
  ...
  furi_hal_gpio_init(&gpio_ext_pa4, GpioModeOutputPushPull, GpioPullUp,
                     GpioSpeedVeryHigh);
  ...
  if (power_reset) {
    ...
    furi_hal_gpio_init_simple(furi_hal_sd_spi_handle->cs,
...
SdSpiStatus sd_get_card_state(void) {
  ...
  response =
      sd_spi_send_cmd(SD_CMD13_SEND_STATUS, 0, 0xFF, SdSpiCmdAnswerTypeR2);
  sd_spi_deselect_card_and_purge();

  // Return status OK if response is valid
  if ((response.r1 == SdSpi_R1_NO_ERROR) &&
      (response.r2 == SdSpi_R2_NO_ERROR ||
       response.r2 == SdSpi_R2_CARD_LOCKED)) {
    return SdSpiStatusOK;
  }
...
SdSpiStatus sd_get_card_info(SD_CardInfo *card_info) {
  ...
  if (sd_high_capacity == 1) {
    ...
    card_info->CardCapacity =
        ((uint64_t)card_info->Csd.version.v2.DeviceSize + 1UL) * 1024UL *
        (uint64_t)card_info->LogBlockSize;
    card_info->LogBlockNbr =
        (card_info->CardCapacity) / (card_info->LogBlockSize);
  } else {
    card_info->CardCapacity = (card_info->Csd.version.v1.DeviceSize + 1);
    card_info->CardCapacity *=
        (1UL << (card_info->Csd.version.v1.DeviceSizeMul + 2));
    card_info->LogBlockSize = 512;
...
SdSpiStatus sd_set_pwd(char *pwd) {
  ...
  for (uint8_t i = 0; i < LOCK_UNLOCK_ATTEMPS; i++) {
    ...
    response =
        sd_spi_send_cmd(SD_CMD42_LOCK_UNLOCK, 0, 0xFF, SdSpiCmdAnswerTypeR1);
    if (response.r1 == SdSpi_R1_NO_ERROR) {
...
SdSpiStatus sd_read_blocks(uint32_t *data, uint32_t address, uint32_t blocks,
                           uint32_t timeout_ms) {
  SdSpiStatus status =
      sd_spi_cmd_read_blocks(data, address, blocks, timeout_ms);
  return status;
}

SdSpiStatus sd_write_blocks(uint32_t *data, uint32_t address, uint32_t blocks,
                            uint32_t timeout_ms) {
  SdSpiStatus status =
      sd_spi_cmd_write_blocks(data, address, blocks, timeout_ms);
  return status;
}
