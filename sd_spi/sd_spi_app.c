...
void app_scene_on_enter_main_menu(void *context) {
  ...
  submenu_add_item(app->menu, "SD Init", AppMenuSelection_Init,
                   app_menu_callback_main_menu, app);
  submenu_add_item(app->menu, "SD Status", AppMenuSelection_Status,
                   app_menu_callback_main_menu, app);
  submenu_add_item(app->menu, "SD Lock", AppMenuSelection_SDLock,
                   app_menu_callback_main_menu, app);
  submenu_add_item(app->menu, "SD Unlock", AppMenuSelection_SDUnLock,
                   app_menu_callback_main_menu, app);
  submenu_add_item(app->menu, "SD Force Erase", Confirmation_Dialog,
                   app_menu_callback_main_menu, app);
  submenu_add_item(app->menu, "Password", AppMenuSelection_Password,
                   app_menu_callback_main_menu, app);
  submenu_add_item(app->menu, "About", AppMenuSelection_Info,
                   app_menu_callback_main_menu, app);
...
void text_input_done_callback(void *context) {
  ...
  if (!storage_file_exists(storage, EXT_PATH("apps_data"))) {
    storage_common_mkdir(storage, EXT_PATH("apps_data"));
  }
  if (!storage_file_exists(storage, EXT_PATH("apps_data/sdspi"))) {
    storage_common_mkdir(storage, EXT_PATH("apps_data/sdspi"));
  }
...
void app_scene_on_enter_password(void *context) {
  ...
  text_input_set_result_callback(app->text_input, text_input_done_callback, app,
                                 app->input_pwd, PASSWORD_MAX_LEN + 1, false);
...
void app_scene_on_enter_dialog(void *context) {
  ...
  dialog_ex_set_header(app->dialog, "Erase SD card?", 128 / 2, 12, AlignCenter,
                       AlignTop);
...
void app_scene_on_enter_info(void *context) {
  ...
  widget_add_text_box_element(
      app->widget_about, 0, 2, 128, 14, AlignCenter, AlignBottom,
      "\e#\e!            SD Spi           \e!\n", false);
  ...
  widget_add_text_scroll_element(app->widget_about, 0, 16, 128, 50,
...
void (*const app_scene_on_enter_handlers[])(void *) = {
    app_scene_on_enter_main_menu, app_scene_on_enter_status,
    // app_scene_on_enter_info,
    app_scene_on_enter_dialog, app_scene_on_enter_password,
    app_scene_on_enter_info};

/** collection of all scene on event handlers - in the same order as their enum
 */
bool (*const app_scene_on_event_handlers[])(void *, SceneManagerEvent) = {
    app_scene_on_event_main_menu, app_scene_on_event_status,
    // app_scene_on_event_info,
    app_scene_on_event_dialog, app_scene_on_event_password,
    app_scene_on_event_info};

/** collection of all scene on exit handlers - in the same order as their enum
 */
void (*const app_scene_on_exit_handlers[])(void *) = {
    app_scene_on_exit_main_menu, app_scene_on_exit_status,
    // app_scene_on_exit_info,
    app_scene_on_exit_dialog, app_scene_on_exit_password,
    app_scene_on_exit_info};
...
bool app_scene_manager_custom_event_callback(void *context,
...
void app_view_dispatcher_init(SDSPIApp *app) {
  ...
  view_dispatcher_add_view(app->view_dispatcher, AppView_Status,
                           text_box_get_view(app->tb_status));

  FURI_LOG_D(TAG, "app_view_dispatcher_init adding view dialog");
  view_dispatcher_add_view(app->view_dispatcher, AppView_Dialog,
                           dialog_ex_get_view(app->dialog));

  FURI_LOG_D(TAG, "app_view_dispatcher_init adding view password");
  view_dispatcher_add_view(app->view_dispatcher, AppView_TextInput,
                           text_input_get_view(app->text_input));

  FURI_LOG_D(TAG, "app_view_dispatcher_init adding view about");
  view_dispatcher_add_view(app->view_dispatcher, AppView_Info,
